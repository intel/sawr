 ...
 *    implemented on the Edison should use the topic prefix "base_"
 *    and none on the UP should (except for base_bridge)
 */
#include "ros/ros.h"
...
ros::Subscriber* base_odom_sub = 0;
...
ros::Publisher* base_cmd_vel_pub = 0;
...
// Callback for base_odom "Odometry" subscription
void base_odom_callback(
   const nav_msgs::OdometryConstPtr& base_odom_msg
) {
   ros::Time current_time = ros::Time::now();
   // Copy to make some tweaks
   nav_msgs::Odometry new_odom_msg(*base_odom_msg);
   ...
   // odom_trans.header.stamp = base_odom_msg->header.stamp; // times need to be well synched!
   odom_trans.header.stamp = current_time; // some lag, but avoids time sync issues
   odom_trans.header.frame_id = "odom";
   odom_trans.child_frame_id = "base_footprint"; 
 
   odom_trans.transform.translation.x = base_odom_msg->pose.pose.position.x;
   odom_trans.transform.translation.y = base_odom_msg->pose.pose.position.y;
   odom_trans.transform.translation.z = base_odom_msg->pose.pose.position.z;
   odom_trans.transform.rotation = base_odom_msg->pose.pose.orientation;
...
void cmd_vel_callback(
   ...
   // Republish raw msg
   base_cmd_vel_pub->publish(cmd_vel_msg);
}
...
int main (
    ...
    // Publish base_cmd_vel "Twist" topic 
    base_cmd_vel_pub = new ros::Publisher(nh.advertise<geometry_msgs::Twist>(
        "base_cmd_vel", 
        100          // queue size
    ));     
    ...
    // Subscribe to base_odom "Odometry" topic 
    base_odom_sub = new ros::Subscriber(nh.subscribe<nav_msgs::Odometry>(
        "base_odom", 
        100,         // queue size  
        &base_odom_callback
    ));
